...
const priorityColors: Record<string, string> = { high: 'badge-red', medium: 'badge-amber', low: 'badge-cyan' }
const agentLabels: Record<string, string> = { cmo: 'CMO', sns: 'SNS', lp: 'LP改善', analytics: '分析', notify: '通知' }
...
export default function ProposalFeed({ proposals: initial }: { proposals: AiProposal[] }) {
  ...
  async function handleAction(id: string, action: 'approved' | 'rejected') {
    ...
    setProposals((p: AiProposal[]) => p.filter(x => x.id !== id))
  ...
  return (
    <div className="card">
      ...
      {proposals.length === 0 ? (
        <div className="text-center py-10 text-[#6b6b8a] text-sm">現在、承認待ちの提案はありません</div>
      ) : (
            ...
            <div key={p.id} className="bg-[#0f0f1e] border border-[#1a1a2e] rounded-xl p-4">
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className={`badge ${priorityColors[p.priority] || 'badge-cyan'}`}>
                    {p.priority === 'high' ? '重要' : p.priority === 'medium' ? '通常' : '低'}
                  </span>
                  <span className="badge badge-cyan">{agentLabels[p.agent] || p.agent}</span>
                </div>
              ...
              {p.expected_impact && <div className="text-xs text-[#00e5a0] mb-3">期待効果: {p.expected_impact}</div>}
